...
class SqlOutputContentProvider {
    ...
    rootRequestHandler(req, res) {
        ...
        let extensionFontFamily = pgsqlConfig.get(Constants.extConfigResultFontFamily).split('\'').join('').split('"').join('');
        let extensionFontSize = pgsqlConfig.get(Constants.extConfigResultFontSize);
